...
## HUD overlay that shows the current section name with a dramatic fade-in
## when the player enters a new section.
...
func _ready():
	...
	hide()
	# Connect after one frame to ensure SectionManager is ready
...
func _process(_delta: float):
	...
	depth_label.text = "Depth: %d m" % max(player_y, 0)
...
func _on_section_changed(_old_section: String, new_section: String):
	...
	# Dramatic fade-in then hold then fade-out
