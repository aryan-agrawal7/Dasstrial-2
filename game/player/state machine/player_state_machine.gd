...
func _on_place_block(block: Block, block_state: Block.State, tile_pos: Vector2i, _ingredients: Array[InventoryItem]):
	player.get_world().set_block(block, tile_pos, block_state)


func _on_building_build_entity(block_entity_definition: BlockEntityDefinition, tile_pos: Vector2i, _ingredients: Array[InventoryItem]):
